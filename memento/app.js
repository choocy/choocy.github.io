...
function mapMemory(row, members = [], media = [], inviteRow = null) {
  ...
  return {
    ...
    uploadedPhotos: media.filter((item) => item.media_type === 'photo').length,
...
function guestMenu() {
  ...
  const appAction = state.inviteCode ? `<a class="guest-menu-action" href="${escapeHtml(appInviteUrl(state.inviteCode, currentHandoffGuestToken()))}" data-open-memento-app><span>Continue to app</span>${icon('arrow-right')}</a>` : '';
  return `
...
async function copyInviteForBrowser(event) {
  ...
  try {
    await navigator.clipboard?.writeText(url);
    state.scannerGateCopied = true;
    state.joinError = '';
  } catch {
    state.scannerGateCopied = false;
